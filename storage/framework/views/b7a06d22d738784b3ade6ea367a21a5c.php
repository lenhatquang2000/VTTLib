...
<div class="max-w-4xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 duration-500">
    ...
    <div class="flex items-center justify-between">
        <div>
            <a href="<?php echo e(route('admin.users.index')); ?>" class="group inline-flex items-center text-sm font-bold text-slate-400 hover:text-indigo-600 transition-colors mb-2">
                ...
                <?php echo e(__('Return to Subject Records')); ?>

            </a>
            <h1 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight"><?php echo e(__('Modify Identity')); ?></h1>
            <p class="text-slate-500 font-medium"><?php echo e(__('Updating access parameters for')); ?> <span class="text-indigo-600 font-bold"><?php echo e($user->email); ?></span></p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-700 overflow-hidden">
        <form action="<?php echo e(route('admin.users.update', $user->id)); ?>" method="POST" class="p-8 md:p-12">
            <?php echo csrf_field(); ?>
            ...
            <div class="space-y-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        <label class="block text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest pl-1"><?php echo e(__('Full Name')); ?></label>
                    ...
                    <div class="space-y-2">
                        <label class="block text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest pl-1"><?php echo e(__('Username')); ?></label>
                        <input type="text" name="username" value="<?php echo e(old('username', $user->username)); ?>" required
                               class="block w-full px-5 py-4 bg-slate-50 dark:bg-slate-950 border-2 border-transparent rounded-2xl focus:bg-white dark:focus:bg-slate-900 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all duration-200 font-bold text-slate-700 dark:text-slate-200">
                    </div>

                    <div class="space-y-2">
                        <label class="block text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest pl-1"><?php echo e(__('Communication Relay')); ?> (Email)</label>
                        <input type="email" name="email" value="<?php echo e(old('email', $user->email)); ?>" required
                               class="block w-full px-5 py-4 bg-slate-50 dark:bg-slate-950 border-2 border-transparent rounded-2xl focus:bg-white dark:focus:bg-slate-900 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all duration-200 font-bold text-slate-700 dark:text-slate-200">
                    </div>

                    <div class="space-y-2">
                        <label class="block text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest pl-1"><?php echo e(__('New Master Cipher')); ?> (Password)</label>
                        <input type="password" name="password" id="password_input" placeholder="<?php echo e(__('Leave blank to keep current')); ?>"
                               class="block w-full px-5 py-4 bg-slate-50 dark:bg-slate-950 border-2 border-transparent rounded-2xl focus:bg-white dark:focus:bg-slate-900 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all duration-200 font-bold text-slate-700 dark:text-slate-200">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="p-6 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center">
            ...
            <div>
                <p class="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest"><?php echo e(__('Enrolled')); ?></p>
                <p class="text-sm font-bold text-slate-700 dark:text-slate-300"><?php echo e($user->created_at ? $user->created_at->format('M d, Y') : 'N/A'); ?></p>
            </div>
        </div>
        <div class="p-6 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center">
            ...
            <div>
                <p class="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest"><?php echo e(__('Current State')); ?></p>
                <p class="text-sm font-bold text-slate-700 dark:text-slate-300 capitalize"><?php echo e($user->status); ?></p>
            </div>
        </div>
        <div class="p-6 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center">
            ...
            <div>
                <p class="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest"><?php echo e(__('Security Roles')); ?></p>
                <p class="text-sm font-bold text-slate-700 dark:text-slate-300"><?php echo e($user->roles->count()); ?> active zones</p>
            </div>
        </div>
    </div>
</div>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\Workspace\VTTU\Laravel\VTTLib\resources\views/admin/users/edit.blade.php ENDPATH**/ ?>