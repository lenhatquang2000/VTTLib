...
<div class="space-y-8 animate-in fade-in duration-700">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div class="flex items-center space-x-4">
            ...
            <div>
                <h1 class="text-3xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight"><?php echo e(__('Document_Types')); ?></h1>
                <p class="text-slate-500 dark:text-slate-400 font-medium mt-1"><?php echo e(__('Manage_document_types_for_library_materials')); ?></p>
            </div>
        </div>
        <button @click="$dispatch('open-modal', 'add-doc-type')" class="inline-flex items-center px-6 py-3 bg-indigo-600 text-white font-black rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200 dark:shadow-none text-[10px] uppercase tracking-widest">
            ...
            <?php echo e(__('Add_Document_Type')); ?>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800 text-left">
                                ...
                                <th class="px-6 py-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest"><?php echo e(__('Document Type')); ?></th>
                                <th class="px-6 py-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest"><?php echo e(__('Code')); ?></th>
                                <th class="px-6 py-4 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest"><?php echo e(__('MARC Type')); ?></th>
                                <th class="px-6 py-4 text-center text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest"><?php echo e(__('Loan_Days')); ?></th>
                                <th class="px-6 py-4 text-center text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest"><?php echo e(__('Loanable')); ?></th>
                        ...
                        <tbody class="divide-y divide-slate-50 dark:divide-slate-800" id="sortable-doc-types">
                            <?php $__empty_1 = true; $__currentLoopData = $documentTypes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $type): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors group" data-id="<?php echo e($type->id); ?>">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center space-x-3">
                                        <div class="w-10 h-10 rounded-xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-400 group-hover:bg-indigo-50 dark:group-hover:bg-indigo-500/10 group-hover:text-indigo-600 transition-all">
                                            <?php if($type->icon): ?>
                                                <i data-lucide="<?php echo e($type->icon); ?>" class="w-5 h-5"></i>
                                            <?php else: ?>
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                            <?php endif; ?>
                                        </div>
                                        <div>
                                            <div class="text-sm font-bold text-slate-900 dark:text-slate-100"><?php echo e($type->name); ?></div>
                                            <div class="text-xs text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-1 max-w-[200px]"><?php echo e($type->description); ?></div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <span class="text-[10px] font-black uppercase tracking-widest text-slate-400 whitespace-nowrap">
                                        <?php echo e($type->marc_type ?: 'Not defined'); ?>

                                    </span>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <div class="inline-flex flex-col items-center">
                                        <span class="text-sm font-black text-slate-800 dark:text-slate-200"><?php echo e($type->default_loan_days); ?></span>
                                        <span class="text-[8px] font-black text-slate-400 uppercase tracking-widest"><?php echo e(__('Days')); ?></span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <span class="inline-flex items-center px-2 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider <?php echo e($type->is_loanable ? 'bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400' : 'bg-slate-50 text-slate-400 dark:bg-slate-800 dark:text-slate-500'); ?>">
                                        <?php echo e($type->is_loanable ? __('Yes') : __('No')); ?>

                                    </span>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <span class="inline-flex px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider <?php echo e($type->is_active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400' : 'bg-rose-100 text-rose-700 dark:bg-rose-500/10 dark:text-rose-400'); ?>">
                                        <?php echo e($type->is_active ? __('Active') : __('Inactive')); ?>

                                    </span>
                                </td>
                                <td class="px-6 py-4 text-right space-x-2 whitespace-nowrap">
                                    <button @click="$dispatch('open-modal', 'edit-doc-type'); $dispatch('set-edit-doc-type', <?php echo \Illuminate\Support\Js::from($type)->toHtml() ?>)" class="p-2 text-slate-400 hover:text-amber-600 transition-colors bg-slate-50 dark:bg-slate-800 rounded-lg" title="<?php echo e(__('Edit')); ?>">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </button>
                                    <form action="<?php echo e(route('admin.document-types.destroy', $type)); ?>" method="POST" class="inline-block" onsubmit="return confirm(<?php echo \Illuminate\Support\Js::from(__('Delete_this_document_type?'))->toHtml() ?>)">
                            ...
                            <tr>
                                <td colspan="8" class="px-6 py-12 text-center text-slate-400 dark:text-slate-500">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-sm font-medium"><?php echo e(__('No_document_types_found')); ?></p>
                                    </div>
                                </td>
                            </tr>
                            <?php endif; ?>
        ...
        <div class="lg:col-span-12">
             <div class="bg-indigo-50 dark:bg-indigo-900/20 p-8 rounded-[3rem] border border-indigo-100 dark:border-indigo-500/20">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div class="max-w-2xl">
                        <h3 class="text-xl font-black text-indigo-900 dark:text-indigo-300 mb-2 uppercase tracking-tight"><?php echo e(__('Understanding_Loan_Days')); ?></h3>
                        <p class="text-indigo-600 dark:text-indigo-400/80 text-sm font-medium leading-relaxed">
                            <?php echo e(__('Loan_Days_defined_here_acts_as_the_base_duration_for_circulation')); ?>. 
                            <?php echo e(__('It_ensures_that_items_of_this_type_have_a_sensible_default_if_no_specific_policy_is_assigned')); ?>.
                        </p>
                    </div>
                </div>
                
                <div class="mt-8">
                    <h4 class="text-[10px] font-black text-indigo-400 dark:text-indigo-500 uppercase tracking-[0.2em] mb-4"><?php echo e(__('MARC21_Type_Reference')); ?> (Leader/06)</h4>
                    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
                        ...
                        ?>
                        <?php $__currentLoopData = $marcCodes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $code => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="bg-white dark:bg-slate-900 px-4 py-3 rounded-2xl border border-indigo-100 dark:border-indigo-500/10 flex items-center space-x-2">
                                <code class="text-xs font-black text-indigo-600 dark:text-indigo-400 font-mono"><?php echo e($code); ?></code>
                                <span class="text-[10px] text-slate-500 dark:text-slate-400 font-bold tracking-tight"><?php echo e(__($label)); ?></span>
                            </div>
...
<div x-data="{ 
    ...
    <template x-if="showAdd">
        <div class="fixed inset-0 z-[100] overflow-y-auto px-4 py-6 sm:px-0 flex items-center justify-center animate-in fade-in duration-300">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden w-full max-w-xl relative">
                <form action="<?php echo e(route('admin.document-types.store')); ?>" method="POST" class="p-8 md:p-10">
                    <?php echo csrf_field(); ?>
                    <div class="flex justify-between items-center mb-8">
                        <div>
                            <h3 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight uppercase"><?php echo e(__('Expand Typology')); ?></h3>
                            <p class="text-slate-500 text-xs font-bold"><?php echo e(__('Define a new category of library material')); ?></p>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                             <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] pl-1"><?php echo e(__('Classification Name')); ?> *</label>
                        ...
                        <div class="space-y-2">
                             <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] pl-1"><?php echo e(__('System Code')); ?> *</label>
                        ...
                        <div class="space-y-2 text-left">
                             <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] pl-1"><?php echo e(__('MARC Selector')); ?> (L/06)</label>
                        ...
                        <div class="space-y-2">
                             <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] pl-1"><?php echo e(__('Glyph Identifier')); ?> (Icon)</label>
                        ...
                        <div class="md:col-span-2 space-y-2">
                             <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] pl-1"><?php echo e(__('Typology Description')); ?></label>
                        ...
                        <div class="space-y-2">
                             <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] pl-1"><?php echo e(__('Base Loan Retention')); ?> (Days)</label>
                        ...
                        <div class="space-y-2">
                             <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] pl-1"><?php echo e(__('Sequence Rank')); ?></label>
                        ...
                        <div class="md:col-span-2 flex items-center space-x-6 px-2">
                            <label class="flex items-center space-x-3 cursor-pointer group">
                                ...
                                <span class="text-xs font-black text-slate-500 uppercase tracking-widest group-hover:text-indigo-600 transition-colors"><?php echo e(__('Allow Circulation')); ?></span>
                            </label>
                            <label class="flex items-center space-x-3 cursor-pointer group">
                                ...
                                <span class="text-xs font-black text-slate-500 uppercase tracking-widest group-hover:text-indigo-600 transition-colors"><?php echo e(__('Operational')); ?></span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="mt-10 flex items-center justify-between gap-4 border-t border-slate-50 dark:border-slate-800 pt-8">
                        ...
                        <button type="submit" class="px-10 py-4 bg-indigo-600 text-white font-black rounded-3xl shadow-xl shadow-indigo-200 dark:shadow-none hover:bg-indigo-700 transition-all uppercase tracking-widest text-[10px]">
                            <?php echo e(__('Commit Definition')); ?>
    ...
    <template x-if="showEdit">
        <div class="fixed inset-0 z-[100] overflow-y-auto px-4 py-6 sm:px-0 flex items-center justify-center animate-in fade-in duration-300">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden w-full max-w-xl relative">
                <form :action="'<?php echo e(url('topsecret/document-types')); ?>/' + docType.id" method="POST" class="p-8 md:p-10">
                    <?php echo csrf_field(); ?> <?php echo method_field('PUT'); ?>
                    <div class="flex justify-between items-center mb-8">
                        <div>
                            <h3 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight uppercase"><?php echo e(__('Modify Typology')); ?></h3>
                            <p class="text-slate-500 text-xs font-bold"><?php echo e(__('Updating definition for')); ?> <span class="text-indigo-600" x-text="docType.name"></span></p>
                        </div>
        ...
        if (el) {
            Sortable.create(el, {
                ...
                onEnd: function() {
                    ...
                    fetch('<?php echo e(route('admin.document-types.order')); ?>', {
                        method: 'POST',
                        headers: {
                    ...
                    .then(data => {
                        if (data.success) {
                            window.dispatchEvent(new CustomEvent('toast', {
                                detail: { message: '<?php echo e(__("Order updated successfully")); ?>', type: 'success' }
                            }));
                        }
                    });
                }
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\Workspace\VTTU\Laravel\VTTLib\resources\views/admin/document-types/index.blade.php ENDPATH**/ ?>