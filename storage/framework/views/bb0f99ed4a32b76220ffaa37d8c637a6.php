...
<body
    ...
    <aside :class="sidebarOpen ? 'w-72' : 'w-24 px-2'"
        ...
        <nav class="flex-1 px-4 py-8 space-y-2.5 overflow-y-auto custom-scrollbar overflow-x-hidden">
            ...
            <div class="space-y-1.5" x-data="{ open: <?php echo e($isParentActive ? 'true' : 'false'); ?> }">
                <button @click="sidebarOpen ? (open = !open) : (sidebarOpen = true, open = true)"
                    ...
                    <div class="flex items-center">
                        ...
                        <span x-show="sidebarOpen" x-cloak
                            class="ml-3 font-bold text-[11px] uppercase tracking-widest whitespace-nowrap"><?php echo e($tab->display_name); ?></span>
                    </div>
                ...
                <div x-show="open && sidebarOpen" x-cloak x-collapse class="px-2 pb-2 space-y-1 bg-slate-50/50 dark:bg-slate-950/30 rounded-2xl mt-1">
                    <?php $__currentLoopData = $assignedChildren; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $child): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <a href="<?php echo e($child->route_name && $child->route_name !== '#' && Route::has($child->route_name) ? route($child->route_name) : '#'); ?>"
                            class="group flex items-center px-3 py-2 text-sm font-medium rounded-lg transition-all duration-200 <?php echo e(request()->routeIs($child->route_name . '*') ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-900/20 dark:text-indigo-400' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-slate-200'); ?>">
                            <div class="flex items-center">
                                <div class="flex-shrink-0">
                                    <i class="<?php echo e($child->icon ?? 'fas fa-circle'); ?> w-5 h-5 flex items-center justify-center text-[8px] opacity-40 group-hover:opacity-100 transition-all"></i>
                                </div>
                                <span class="ml-3 truncate font-bold text-[10px] uppercase tracking-widest"><?php echo e($child->display_name); ?></span>
                            </div>
                        </a>
            ...
            <?php else: ?>
            <a href="<?php echo e((!blank($tab->route_name) && $tab->route_name !== '#' && Route::has($tab->route_name)) ? route($tab->route_name) : '#'); ?>"
