    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden">
            ...
            <form method="GET" class="flex flex-col lg:flex-row gap-4 items-center justify-between">
                ...
                <div class="flex items-center gap-3 w-full lg:w-auto">
                    ...
                    <?php if(request()->anyFilled(['search', 'status'])): ?>
                        <a href="<?php echo e(route('admin.announcements.index')); ?>" class="flex-1 lg:flex-none px-6 py-2.5 bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-400 rounded-xl text-sm font-bold hover:bg-gray-200 transition-all text-center">
                            <?php echo e(__('Clear')); ?>
        ...
        <div class="p-6 bg-gray-50/30 dark:bg-slate-900/50 border-b border-gray-50 dark:border-slate-800 flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="text-xs font-bold text-gray-400 dark:text-slate-500 tracking-widest uppercase">
                <?php echo e(__('Found :count announcements', ['count' => $announcements->total()])); ?>
                    ...
                    <tbody class="divide-y divide-gray-50 dark:divide-slate-800" id="announcement-table-body">
                        <?php $__currentLoopData = $announcements; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr class="hover:bg-gray-50/80 dark:hover:bg-slate-800/50 transition-colors group cursor-pointer" 
                                ...
                                onclick="window.location.href='<?php echo e(route('admin.announcements.edit', $item)); ?>'">
                                ...
                                <td class="px-6 py-4" onclick="event.stopPropagation()">
                                    <input type="checkbox" class="announcement-checkbox rounded text-indigo-600 focus:ring-0 w-4 h-4 cursor-pointer" value="<?php echo e($item->id); ?>">
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-right" onclick="event.stopPropagation()">
                                    <div class="flex items-center justify-end space-x-1">
                                        <a href="<?php echo e(route('admin.announcements.edit', $item)); ?>" 
                                           class="p-2 text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-900/20 rounded-lg transition-colors" title="Sửa">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="<?php echo e(route('admin.announcements.destroy', $item)); ?>" 
                                              method="POST" 
...
function executeBulkAction(action) {
    ...
    fetch('<?php echo e(route("admin.announcements.bulk-action")); ?>', {
        method: 'POST',
        headers: {
...
function updateOrder(ids) {
    fetch('<?php echo e(route("admin.announcements.reorder")); ?>', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
function autoGenerateAnnouncement() {
    Swal.fire({
        ...
        preConfirm: () => {
            return fetch('<?php echo e(route("admin.announcements.auto-generate")); ?>', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>'
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\Workspace\VTTU\Laravel\VTTLib\resources\views/admin/announcements/index.blade.php ENDPATH**/ ?>