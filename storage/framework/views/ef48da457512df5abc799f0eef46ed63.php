...
<div class="space-y-[6px] animate-fade-in">
    ...
    <div class="bg-card border border-border shadow-sm p-3 flex flex-col sm:flex-row gap-2 transition-colors duration-200">
        <div class="relative min-w-[140px]">
            <select class="w-full h-9 pl-3 pr-8 text-xs font-bold bg-muted/30 border border-border rounded-sm appearance-none outline-none focus:ring-1 focus:ring-vttu-red/30 focus:border-vttu-red/30 transition-all cursor-pointer">
                <option value="title"><?php echo e(__('Tiêu đề')); ?></option>
                <option value="author"><?php echo e(__('Tác giả')); ?></option>
                <option value="subject"><?php echo e(__('Chủ đề')); ?></option>
            </select>
        ...
        <div class="flex-1 relative group">
            <input type="text" 
                   placeholder="<?php echo e(__('Nhập từ khóa tìm kiếm tài liệu số...')); ?>"
        ...
        <div class="flex gap-1.5 h-9">
            <button class="flex-1 sm:flex-none px-5 bg-vttu-red text-white rounded-sm hover:bg-vttu-dark active:scale-[0.97] transition-all shadow-md shadow-vttu-red/20 flex items-center justify-center">
                ...
                <span class="text-xs font-black uppercase tracking-wider"><?php echo e(__('Tìm')); ?></span>
            </button>
    ...
    <div class="flex items-center gap-2 text-vttu-red px-1 pt-1">
        ...
        <span class="text-[11px] font-black uppercase tracking-[0.15em]">
            <?php echo e(__('Tổng số')); ?>: <span class="text-sm ml-0.5"><?php echo e(number_format($totalCount)); ?></span>
        </span>
    </div>
        ...
        <table class="w-full text-left border-collapse table-fixed">
            <thead class="bg-muted/50 text-[10px] font-black uppercase tracking-widest text-muted-foreground border-b border-border">
                <tr>
                    ...
                    <th class="py-2.5 px-4"><?php echo e(__('Tiêu đề')); ?></th>
                    <th class="py-2.5 px-4 w-48 text-center hidden md:table-cell border-l border-border/50"><?php echo e(__('Tác giả')); ?></th>
                    <th class="py-2.5 px-4 w-40 text-right hidden sm:table-cell border-l border-border/50"><?php echo e(__('Loại tài liệu')); ?></th>
                </tr>
            </thead>
            <tbody class="divide-y divide-border/40 text-xs">
                <?php $__empty_1 = true; $__currentLoopData = $resources; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <tr class="group hover:bg-muted/30 active:bg-muted/50 transition-colors cursor-pointer"
                    onclick="window.location.href='<?php echo e(route('site.digital-resources.show', $item->id)); ?>'">
                    <td class="py-4 px-4 text-center font-bold text-muted-foreground group-hover:text-vttu-red transition-colors border-r border-border/30 w-12">
                        <?php echo e($resources->firstItem() + $index); ?>
                    ...
                    <td class="py-4 px-4">
                        <div class="flex items-center gap-4">
                            <div class="w-16 h-20 bg-muted rounded-sm flex-shrink-0 overflow-hidden border border-border group-hover:border-vttu-red/30 transition-colors shadow-sm relative">
                                <img src="<?php echo e($item->thumbnail_url ?? 'https://placehold.co/100x140/7B0000/FFFFFF?text=DOC'); ?>" 
                            ...
                            <div class="space-y-2 overflow-hidden flex-1">
                                ...
                                <div class="flex flex-wrap gap-2 pt-1 md:hidden">
                                    <span class="text-[10px] text-vttu-red/70 font-bold italic"><i data-lucide="user" class="w-3 h-3 inline mr-0.5"></i> <?php echo e($item->author ?: __('Đang cập nhật')); ?></span>
                                    <span class="text-[10px] text-muted-foreground font-bold italic"><i data-lucide="file-text" class="w-3 h-3 inline mr-0.5"></i> <?php echo e(__('Tài liệu số')); ?></span>
                                </div>
                            </div>
                        </div>
                    </td>
                    <td class="py-4 px-4 w-48 text-center hidden md:table-cell border-l border-border/30">
                        <span class="font-black text-vttu-dark/70 group-hover:text-vttu-dark transition-colors uppercase tracking-tight text-[11px] line-clamp-2">
                            <?php echo e($item->author ?: __('Đang cập nhật')); ?>
                ...
                <tr>
                    <td colspan="4" class="py-20 text-center">
                        <div class="flex flex-col items-center justify-center opacity-20">
                            ...
                            <p class="text-sm font-bold uppercase tracking-widest"><?php echo e(__('Không tìm thấy tài liệu nào')); ?></p>
                        </div>
                    </td>
                </tr>
                <?php endif; ?>
    ...
    <!-- Pagination -->
    <?php if($resources instanceof \Illuminate\Pagination\LengthAwarePaginator && $resources->hasPages()): ?>
        <div class="pt-4 px-1">
