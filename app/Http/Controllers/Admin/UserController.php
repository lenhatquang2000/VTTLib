...
class UserController extends Controller
{
    ...
    /**
     * Unified User Management - Main Users Table
     */
    public function index(Request $request)
    {
        $search = $request->query('search');
        $roleId = $request->query('role_id');
        $activeRole = $request->query('role'); // For legacy admin-style filtering if needed
        $perPage = $request->query('per_page', 10);

        // If 'role' name is provided instead of ID (legacy support)
        if ($activeRole && $activeRole !== 'all' && !$roleId) {
    ...
    public function privileges(Request $request)
    {
        ...
        return view('admin.users.privileges', compact('roleUsers', 'roles', 'sidebars', 'search', 'roleId', 'perPage', 'stats'));
    }
    ...
    public function update(UpdateUserRequest $request, $id)
    {
        try {
            ...
            // Legacy support for role syncing if sent in the request
