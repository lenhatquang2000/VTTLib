...
$baseDir = __DIR__ . '/public/tailieuso';
if (!is_dir($baseDir)) {
    echo "Thu muc public/tailieuso khong ton tai!" . PHP_EOL;
    exit(1);
}

// Lay tat ca cac thu muc con trong public/tailieuso
$dirs = array_filter(glob($baseDir . '/*'), 'is_dir');

echo "Tim thay " . count($dirs) . " thu muc con trong public/tailieuso." . PHP_EOL;
...
foreach ($dirs as $dirPath) {
    ...
    // Kiem tra xem tai lieu nay da ton tai trong CSDL chua
    $existing = DigitalResource::where('title', $title)
        ->where('folder_id', $folder->id)
        ->first();

    if ($existing) {
        echo "Tai lieu '$title' da ton tai trong thu muc '{$folder->folder_name}'. Bo qua." . PHP_EOL;
        $skippedCount++;
        continue;
    }

    // Xy ly File dinh kem (PDF)
    $pdfFileName = isset($meta['tap_tin_dinh_kem']) ? trim($meta['tap_tin_dinh_kem']) : '';
    if (empty($pdfFileName)) {
        // Thu quet file pdf trong thu muc neu meta trong
    ...
    if (!empty($pdfFileName) && file_exists($sourcePdfPath)) {
        ...
        echo "Loi thu muc $folderId: Khong tim thay file PDF dinh kem." . PHP_EOL;
        $skippedCount++;
        continue;
    }
    ...
    DigitalResource::create([
        ...
        'coverage' => isset($meta['coverage']) ? trim($meta['coverage']) : null,
    ...
    echo "Import thanh cong: '$title' -> Thu muc '{$folder->folder_name}'" . PHP_EOL;
    $successCount++;
}

echo "=== KET QUA IMPORT ===" . PHP_EOL;
