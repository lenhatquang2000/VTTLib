...
<div class="space-y-6">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-slate-100">{{ __('Thêm thư mục tài liệu số') }}</h1>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
        <form action="{{ route('admin.digital-categories.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="code" required class="input-field" value="{{ old('code') }}">
                    </div>
                </div>
            </div>
