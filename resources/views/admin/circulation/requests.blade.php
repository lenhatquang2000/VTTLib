    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div class="flex gap-2">
            <a href="{{ route('admin.circulation.requests', ['status' => 'pending']) }}" 
               class="px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all {{ $status == 'pending' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400' }}">
                Đang chờ duyệt
            </a>
            <a href="{{ route('admin.circulation.requests', ['status' => 'ready']) }}" 
               class="px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all {{ $status == 'ready' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-500/20' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400' }}">
                Sẵn sàng lấy
            </a>
            <a href="{{ route('admin.circulation.requests', ['status' => 'all']) }}" 
               class="px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all {{ $status == 'all' ? 'bg-slate-800 text-white shadow-lg shadow-slate-900/20' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400' }}">
                Tất cả
            </a>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden">
        <div class="flex border-b border-gray-200 dark:border-slate-700">
            <div class="flex space-x-1 p-2">
                <a href="{{ route('admin.circulation.loan-desk') }}" 
                    ...
                    <i class="fas fa-arrow-right mr-2"></i>{{ __('Mượn sách') }}
                </a>
                <a href="{{ route('admin.circulation.loan-desk') }}?tab=checkin"
                        class="px-6 py-3 text-sm font-medium rounded-lg transition-all duration-200 text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200">
                    <i class="fas fa-arrow-left mr-2"></i>{{ __('Trả sách') }}
                </a>
                <a href="{{ route('admin.circulation.loan-desk') }}?tab=reading-room"
                        class="px-6 py-3 text-sm font-medium rounded-lg transition-all duration-200 text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200">
                    <i class="fas fa-book-reader mr-2"></i>{{ __('Mượn đọc') }}
                </a>
                <a href="{{ route('admin.circulation.loan-desk') }}?tab=hold"
                        class="px-6 py-3 text-sm font-medium rounded-lg transition-all duration-200 text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200">
                    <i class="fas fa-bookmark mr-2"></i>{{ __('Giữ lại') }}
                </a>
                <a href="{{ route('admin.circulation.requests') }}"
                    ...
                    <i class="fas fa-clipboard-list mr-2"></i>{{ __('Yêu cầu mượn') }}
                </a>
            </div>
        </div>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        ...
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                            ...
                            <td class="px-6 py-4 text-center">
                                ...
                                @else
                                    <span class="px-2 py-1 bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-[10px] font-black uppercase rounded-lg">{{ $req->status }}</span>
                                @endif
                            </td>
