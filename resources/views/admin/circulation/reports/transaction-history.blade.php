    ...
    <div class="p-6 border-b border-indigo-200 dark:border-indigo-900/30 flex justify-between items-center bg-indigo-50 dark:bg-indigo-950/20">
        <div>
            <h1 class="text-2xl font-black text-indigo-800 dark:text-indigo-400 uppercase tracking-tight flex items-center">
                ...
                {{ __('Detailed Book Transaction History') }}
            </h1>
            <p class="text-sm text-indigo-600/70 dark:text-indigo-400/50 mt-1">
                {{ __('Monitoring full history of library book circulation.') }}
            </p>
        </div>
        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-indigo-50/50 dark:bg-slate-800/50 text-indigo-600 dark:text-indigo-400/70 text-[10px] font-black uppercase tracking-widest border-b border-indigo-100 dark:border-slate-700">
                    ...
                    <th class="px-6 py-4">{{ __('Return Date') }}</th>
            ...
            <tbody class="divide-y divide-indigo-100 dark:divide-slate-800">
                ...
                <tr class="hover:bg-indigo-50/30 dark:hover:bg-indigo-900/5 transition-colors">
                    ...
                    <td class="px-6 py-4 text-sm font-semibold text-slate-800 dark:text-slate-200">
                        {{ $txn->bookItem->bibliographicRecord->title ?? __('No_Title_Defined') }}
                    </td>
                    <td class="px-6 py-4 text-sm font-semibold text-slate-800 dark:text-slate-200">
                        {{ $txn->patron->user->name ?? 'Unknown' }}
                    </td>
                    <td class="px-6 py-4 text-xs font-medium text-slate-600 dark:text-slate-400">
                        {{ $txn->loan_date ? \Carbon\Carbon::parse($txn->loan_date)->format('d/m/Y H:i') : '-' }}
                    </td>
                    <td class="px-6 py-4 text-xs font-medium text-slate-600 dark:text-slate-400">
                        {{ $txn->return_date ? \Carbon\Carbon::parse($txn->return_date)->format('d/m/Y H:i') : '-' }}
                    </td>
                    <td class="px-6 py-4">
                        @php
                            $statusMap = [
                                'borrowed' => ['label' => __('borrowed'), 'bg' => 'bg-amber-100 dark:bg-amber-900/20', 'text' => 'text-amber-600 dark:text-amber-400'],
                                'returned' => ['label' => __('returned'), 'bg' => 'bg-green-100 dark:bg-green-900/20', 'text' => 'text-green-600 dark:text-green-400'],
                                'overdue' => ['label' => __('overdue'), 'bg' => 'bg-rose-100 dark:bg-rose-900/20', 'text' => 'text-rose-600 dark:text-rose-400'],
                                'lost' => ['label' => __('lost'), 'bg' => 'bg-slate-100 dark:bg-slate-900/20', 'text' => 'text-slate-600 dark:text-slate-400'],
                            ];
                            $st = $statusMap[$txn->status] ?? ['label' => $txn->status, 'bg' => 'bg-slate-100', 'text' => 'text-slate-600'];
                        @endphp
                        <span class="px-2 py-1 text-[9px] font-black uppercase rounded-full {{ $st['bg'] }} {{ $st['text'] }}">
                            {{ $st['label'] }}
                        </span>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="6" class="px-6 py-12 text-center text-indigo-400 italic font-medium">
                        {{ __('No_Records_Found') }}
                    </td>
                </tr>
                @endforelse
