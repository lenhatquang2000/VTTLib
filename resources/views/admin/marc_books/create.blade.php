...
@section('content')
    <div class="space-y-6 w-full pb-20">
        <div class="flex justify-between items-center bg-white dark:bg-slate-900 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800">
            ...
            <div class="flex space-x-3">
                ...
                <button form="catalogForm" type="submit"
                    class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-lg text-sm font-semibold transition shadow-lg shadow-indigo-100 dark:shadow-none">
                    {{ __('Save_Record') }}
                </button>
            </div>
        </div>
        ...
        <form id="catalogForm" action="{{ route('admin.marc.book.store') }}" method="POST" class="space-y-6">
            ...
            <div class="bg-slate-900 dark:bg-slate-900 rounded-xl shadow-xl border border-slate-800 dark:border-slate-800 overflow-hidden" x-data="{ expanded: false }">
                        ...
                        <div x-data="{ 
                            ...
                            <div class="space-y-3">
                                <template x-for="(row, index) in rows" :key="index">
                                    <div class="flex flex-col md:flex-row gap-3 items-start bg-gray-50/30 dark:bg-slate-800/30 p-3 rounded-lg border border-gray-100 dark:border-slate-700 group">
                                        <!-- Subfield Selector -->
                                        <div class="w-full md:w-1/3">
                                            <select :name="'fields[' + '{{ $tag->tag }}' + '][subfields][' + index + '][code]'" 
                                                    x-model="row.code"
                                                    class="w-full border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 dark:text-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 font-mono">
                                        ...
                                        <div class="w-full md:w-2/3 flex gap-2">
                                            <input type="text" 
                                                   ...
                                                   class="flex-1 border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 dark:text-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500">
                                            
                                            <button type="button" @click="rows.splice(index, 1)" class="text-gray-300 dark:text-slate-600 hover:text-rose-500 p-2 transition opacity-0 group-hover:opacity-100">
                            ...
                            <button type="button" @click="rows.push({ code: '', value: '' })" 
                                    class="mt-4 inline-flex items-center text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 uppercase tracking-wider">
            ...
            <div class="bg-gray-50 dark:bg-slate-800/50 p-6 rounded-xl border-2 border-dashed border-gray-200 dark:border-slate-800 text-center">
                <button type="submit"
                    class="bg-indigo-600 hover:bg-indigo-700 text-white px-10 py-4 rounded-xl text-lg font-bold transition shadow-xl shadow-indigo-100 dark:shadow-none uppercase tracking-widest">
                    {{ __('Finalize_Save_Record') }}
                </button>
                <p class="text-xs text-gray-400 dark:text-slate-500 mt-4">{{ __('Save_Note') }}</p>
            </div>
        </form>
    </div>
