            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                <div class="lg:col-span-4 bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden sticky top-6">
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div class="space-y-1">
                            <label class="block text-[10px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-widest">{{ __('Distribution Quantity') }}</label>
                            <input type="number" x-model="batchQuantity" min="1" max="100"
                                ...
                                placeholder="1">
                            <p class="text-[9px] text-gray-400 mt-1 italic">{{ __('Auto-generate items based on this quantity') }}</p>
                        </div>
            ...
            generateBatchItems() {
                if (!this.batchQuantity || this.batchQuantity < 1) {
                    Swal.fire({
                        ...
                        text: '{{ __('Please_enter_valid_quantity') }}',
                ...
                if (!this.newItem.branch_id || !this.newItem.storage_location_id) {
                    Swal.fire({
                        ...
                        text: '{{ __('Please_setup_first_item_before_batch_generation') }}',
                        confirmButtonColor: '#3b82f6',
                        confirmButtonText: '{{ __('Understood') }}'
                    });
                    return;
                ...
                Swal.fire({
                    icon: 'success',
                    title: '{{ __('Batch Generated') }}',
                    text: `{{ __('Successfully generated') }} ${this.batchQuantity} {{ __('items') }}`,
