...
<div class="space-y-8 pb-10">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 gap-4">
        <div>
            <h2 class="text-2xl font-black text-gray-800 dark:text-slate-100 tracking-tight">{{ __('Announcement_Management') }}</h2>
            <p class="text-sm text-gray-500 dark:text-slate-400 mt-1">{{ __('Manage_Announcements_Instruction') }}</p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden">
            ...
            <form method="GET" class="flex flex-col lg:flex-row gap-4 items-center justify-between">
                ...
                <div class="flex items-center gap-3 w-full lg:w-auto">
                    ...
                    @if(request()->anyFilled(['search', 'status']))
                        <a href="{{ route('admin.announcements.index') }}" class="flex-1 lg:flex-none px-6 py-2.5 bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-400 rounded-xl text-sm font-bold hover:bg-gray-200 transition-all text-center">
                            {{ __('Clear') }}
                        </a>
                    @endif
                </div>
        ...
        <div class="p-6 bg-gray-50/30 dark:bg-slate-900/50 border-b border-gray-50 dark:border-slate-800 flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="text-xs font-bold text-gray-400 dark:text-slate-500 tracking-widest uppercase">
                {{ __('Found :count announcements', ['count' => $announcements->total()]) }}
            </div>
        </div>
                ...
                <table class="w-full text-left">
                    <thead>
                        <tr class="bg-gray-50 dark:bg-slate-800/50 text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest border-b border-gray-100 dark:border-slate-800">
                            ...
                            <th class="px-6 py-4">{{ __('Announcement_Title') }}</th>
                            <th class="px-6 py-4">{{ __('Author') }}</th>
                            <th class="px-6 py-4">{{ __('Status') }}</th>
                            <th class="px-6 py-4 text-center">{{ __('Views') }}</th>
                            <th class="px-6 py-4 text-right">{{ __('Actions') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-50 dark:divide-slate-800" id="announcement-table-body">
                        @foreach($announcements as $item)
                            <tr class="hover:bg-gray-50/80 dark:hover:bg-slate-800/50 transition-colors group cursor-pointer" 
                                data-id="{{ $item->id }}"
                                onclick="window.location.href='{{ route('admin.announcements.edit', $item) }}'">
                                ...
                                <td class="px-6 py-4" onclick="event.stopPropagation()">
                                    <input type="checkbox" class="announcement-checkbox rounded text-indigo-600 focus:ring-0 w-4 h-4 cursor-pointer" value="{{ $item->id }}">
                                </td>
                                <td class="px-6 py-4">
        ...
        @else
            ...
            <div class="text-center py-24 bg-white dark:bg-slate-900 border-t border-gray-50 dark:border-slate-800">
                ...
                <h3 class="text-xl font-black text-gray-800 dark:text-slate-100 mb-2 uppercase tracking-tight">{{ __('No_Announcements_Found') }}</h3>
                <p class="text-gray-500 dark:text-slate-400 mb-8 max-w-sm mx-auto text-sm leading-relaxed">{{ __('No_Announcements_Instruction') }}</p>
                <a href="{{ route('admin.announcements.create') }}" 
                   class="inline-flex items-center px-8 py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl text-sm font-black transition-all shadow-xl shadow-indigo-200 dark:shadow-none hover:-translate-y-1">
                    <i class="fas fa-plus mr-2"></i> {{ __('Create_First_Announcement') }}
                </a>
            </div>
        @endif
    </div>
...
function toggleSelectAll() {
    ...
    checkboxes.forEach(checkbox => {
        ...
        if (selectAll) selectedAnnouncements.add(id);
...
function performBulkAction() {
    ...
    if (!action || selectedAnnouncements.size === 0) {
    ...
    if (action === 'delete') {
        Swal.fire({
            title: 'Xác nhận xóa?',
            text: `Bạn có chắc muốn xóa ${selectedAnnouncements.size} thông báo đã chọn?`,
            icon: 'warning',
...
function executeBulkAction(action) {
    ...
    fetch('{{ route("admin.announcements.bulk-action") }}', {
        method: 'POST',
        headers: {
...
document.addEventListener('DOMContentLoaded', function() {
    const el = document.getElementById('announcement-table-body');
    if (el) {
        Sortable.create(el, {
            handle: '.handle',
            animation: 150,
...
function autoGenerateAnnouncement() {
    Swal.fire({
        ...
        preConfirm: () => {
            return fetch('{{ route("admin.announcements.auto-generate") }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
