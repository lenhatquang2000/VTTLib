...
<div class="space-y-8 animate-in fade-in duration-700">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight leading-tight">{{ __('Identity Command Center') }}</h1>
            ...
            <form action="{{ route('admin.users.index') }}" method="GET" class="flex flex-col lg:flex-row gap-4">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <div class="flex items-center bg-slate-100/50 dark:bg-slate-900/50 p-1.5 rounded-2xl mr-4 border border-slate-100 dark:border-slate-700">
                        <a href="{{ route('admin.users.index', array_merge(request()->all(), ['role_id' => null])) }}" 
                           class="px-5 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all {{ !$roleId ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'text-slate-500 hover:text-slate-700' }}">
                            {{ __('All') }}
                        </a>
                        @foreach($roles as $role)
                            <a href="{{ route('admin.users.index', array_merge(request()->all(), ['role_id' => $role->id])) }}" 
                               class="px-5 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all {{ $roleId == $role->id ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'text-slate-500 hover:text-slate-700' }}">
                                {{ $role->display_name }}
                            </a>
                        @endforeach
                    </div>
                    ...
                    <div class="flex gap-2 min-w-full sm:min-w-0">
                        ...
                        @if($search || $roleId)
                            <a href="{{ route('admin.users.index') }}" class="px-6 py-5 bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 rounded-[1.5rem] hover:bg-rose-600 hover:text-white transition flex items-center justify-center border border-rose-100 dark:border-rose-900/30">
                            ...
                            <div class="flex flex-wrap items-center gap-2 text-sky-400">
                                ...
                                <div class="relative group/role">
                                    <span class="inline-flex items-center px-4 py-1.5 text-[9px] font-black uppercase tracking-wider rounded-xl border {{ $role->name == 'root' ? 'bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 border-rose-100 dark:border-rose-900/30 shadow-sm' : 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 border-indigo-100 dark:border-indigo-900/30 shadow-sm' }}">
                                        ...
                                        <form action="{{ route('admin.users.roles.remove', $role->pivot->id) }}" method="POST" class="ml-2 inline-flex items-center">
                                            @csrf @method('DELETE')
                    ...
                    <tr>
                        <td colspan="5" class="px-10 py-32 text-center">
                            <div class="flex flex-col items-center max-w-sm mx-auto">
                                ...
                                <a href="{{ route('admin.users.index') }}" class="mt-10 px-8 py-3 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 font-black text-[10px] uppercase tracking-[0.3em] rounded-2xl hover:bg-indigo-600 hover:text-white transition-all">{{ __('Reset Global Filter') }}</a>
                            </div>
                        </td>
                    </tr>
                    @endforelse
            ...
            <div class="bg-white dark:bg-slate-800 rounded-[3.5rem] shadow-2xl relative overflow-hidden max-h-[90vh] flex flex-col border border-slate-200 dark:border-slate-700 animate-modal">
                <div class="p-10 border-b border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 z-10">
                    <div class="flex justify-between items-start">
                        <div>
                            <h3 class="text-3xl font-black text-slate-900 dark:text-white leading-tight tracking-tight">{{ __('Access Control Terminal') }}</h3>
                            ...
                            @foreach($sidebars as $sidebar)
                                <div class="col-span-1 md:col-span-2 space-y-4 mb-6 last:mb-0">
                                    <label class="flex items-center p-5 bg-white dark:bg-slate-800 rounded-3xl border-2 border-slate-100 dark:border-slate-700 shadow-sm cursor-pointer hover:border-indigo-500 transition-all group">
                                        <input type="checkbox" name="sidebar_ids[]" value="{{ $sidebar->id }}" class="sidebar-checkbox w-6 h-6 rounded-lg text-indigo-600 border-slate-300 dark:border-slate-600">
                                        <div class="ml-4">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 pl-8 border-l-2 border-slate-100 dark:border-slate-700 ml-3">
                                            @foreach($sidebar->children as $child)
                                                <label class="flex items-center p-4 bg-white dark:bg-slate-800 rounded-2xl border border-transparent hover:border-indigo-300 dark:hover:border-indigo-800 cursor-pointer shadow-sm transition-all">
                                                    <input type="checkbox" name="sidebar_ids[]" value="{{ $child->id }}" class="sidebar-checkbox w-4 h-4 rounded text-indigo-600 border-slate-300 dark:border-slate-600">
                                                    <div class="ml-4">
                                                        <span class="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase">{{ __($child->name) }}</span>
                                                    </div>
                                                </label>
                                            @endforeach
                                        </div>
