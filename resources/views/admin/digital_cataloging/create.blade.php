...
<div class="p-4 bg-background min-h-screen font-sans">
    <form id="cataloging-form" action="{{ route('admin.digital-cataloging.store') }}" method="POST" enctype="multipart/form-data" class="max-w-7xl mx-auto grid grid-cols-12 gap-4">
        @csrf
        ...
        <div class="col-span-12 lg:col-span-3 space-y-4">
            ...
            <div class="text-center bg-card border border-border rounded-md p-4 shadow-sm">
                <label class="block text-xs font-bold text-muted-foreground uppercase tracking-wider mb-3">{{ __('Ảnh bìa tài liệu') }}</label>
            ...
            <div class="p-3 bg-card border border-border rounded-md shadow-sm">
                <label class="block text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">{{ __('Vị trí lưu trữ') }}</label>
                <div class="flex items-center gap-2 p-2 bg-muted/50 rounded-md border border-border">
                    ...
                    <div class="min-w-0">
                        ...
                        <div class="text-xs font-bold text-foreground truncate">{{ $folder->folder_name }}</div>
                    </div>
                </div>
        ...
        <div class="col-span-12 lg:col-span-9 space-y-4">
            ...
            <div class="bg-primary/5 border border-primary/10 rounded-md p-3">
                <label class="block text-[10px] font-bold text-primary uppercase tracking-widest mb-2">{{ __('Tệp tài liệu chính (PDF) *') }}</label>
                <label class="flex items-center justify-between px-3 py-2 bg-background border border-border rounded-md cursor-pointer hover:bg-muted/50 transition-all shadow-sm">
                    <div class="flex items-center gap-2 text-foreground font-medium">
                        ...
                        <span id="file-name-display" class="text-xs">{{ __('Chọn tệp PDF (Tối đa 50MB)') }}</span>
                    </div>
                    <span class="px-2 py-1 bg-primary text-[10px] font-bold text-primary-foreground rounded uppercase tracking-wider">{{ __('Chọn file') }}</span>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="col-span-2 space-y-1">
                        <div class="flex items-center gap-2">
                            <label class="text-xs font-bold text-foreground uppercase tracking-wider">{{ __('Tiêu đề tài liệu *') }}</label>
                        </div>
                        <input type="text" name="title" required placeholder="{{ __('Ví dụ: Giáo trình Lập trình Laravel 2026') }}"
                    ...
                    <div class="space-y-1">
                        <label class="block text-xs font-bold text-foreground uppercase tracking-wider">{{ __('Loại tài liệu *') }}</label>
                    ...
                    <div class="space-y-1">
                        <label class="block text-xs font-bold text-foreground uppercase tracking-wider">{{ __('Ngôn ngữ *') }}</label>
                    ...
                    <div class="space-y-1">
                        <label class="text-xs font-bold text-foreground uppercase tracking-wider">{{ __('Tác giả chính') }}</label>
                        <input type="text" name="authors" placeholder="{{ __('Họ và tên tác giả...') }}"
                               class="w-full px-3 py-2 bg-background border border-border rounded text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary transition-all text-xs">
                    </div>

                    <div class="space-y-1">
                        <label class="text-xs font-bold text-foreground uppercase tracking-wider">{{ __('Nhà xuất bản') }}</label>
                        <input type="text" name="publisher" placeholder="{{ __('Tên NXB...') }}"
                               class="w-full px-3 py-2 bg-background border border-border rounded text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary transition-all text-xs">
                    </div>

                    <div class="space-y-1">
                        <label class="text-xs font-bold text-foreground uppercase tracking-wider">{{ __('Năm phát hành') }}</label>
                        <input type="text" name="publish_year" placeholder="{{ __('Ví dụ: 2026') }}"
                               class="w-full px-3 py-2 bg-background border border-border rounded text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary transition-all text-xs">
                    </div>

                    <div class="space-y-1">
                        <label class="text-xs font-bold text-foreground uppercase tracking-wider">{{ __('Định danh (Identifier)') }}</label>
                        <input type="text" name="identifier" placeholder="{{ __('ISBN, ISSN hoặc Mã nội bộ...') }}"
                               class="w-full px-3 py-2 bg-background border border-border rounded text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary transition-all text-xs">
                    </div>

                    <!-- TinyMCE Section -->
                    <div class="col-span-2 space-y-1.5">
                        <label class="block text-xs font-bold text-foreground uppercase tracking-wider">{{ __('Mô tả / Tóm tắt nội dung') }}</label>
                    ...
                    <div class="col-span-2 grid grid-cols-3 gap-3 p-3 bg-muted/30 rounded-md border border-border border-dashed">
                        <div class="space-y-1">
                            <label class="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{{ __('Nguồn gốc') }}</label>
                        ...
                        <div class="space-y-1">
                            <label class="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{{ __('Phạm vi') }}</label>
                        ...
                        <div class="space-y-1">
                            <label class="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{{ __('Bản quyền') }}</label>
                    ...
                    <div class="col-span-2 grid grid-cols-2 gap-4">
                        <div class="flex items-center gap-2 group">
                            ...
                            <div class="flex-1 space-y-0.5">
                                <label class="block text-[9px] font-bold text-muted-foreground uppercase tracking-widest">{{ __('Liên kết tham chiếu (URL)') }}</label>
                        ...
                        <div class="flex items-center gap-2 group">
                            ...
                            <div class="flex-1 space-y-0.5">
                                <label class="block text-[9px] font-bold text-muted-foreground uppercase tracking-widest">{{ __('Liên kết biên mục (MARC)') }}</label>
            ...
            <div class="flex justify-between items-center pt-4 border-t border-border">
                <p class="text-[10px] text-muted-foreground font-medium italic">* {{ __('Các trường đánh dấu sao là bắt buộc.') }}</p>
