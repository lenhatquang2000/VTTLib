...
<div class="space-y-6">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-slate-100">{{ __('Add Media Category') }}</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">{{ __('Create a new category for your media items') }}</p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
        <form action="{{ route('admin.media-categories.store') }}" method="POST">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="name" required
                               ...
                               placeholder="{{ __('e.g: Home Slider') }}"
                               value="{{ old('name') }}">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="code" required
                               class="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:focus:ring-blue-400 dark:focus:border-blue-400 transition-colors duration-200" 
                               placeholder="{{ __('e.g: home_slider') }}"
                               value="{{ old('code') }}">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="position" 
                               class="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:focus:ring-blue-400 dark:focus:border-blue-400 transition-colors duration-200" 
                               placeholder="{{ __('e.g: home, header, sidebar') }}"
                               value="{{ old('position') }}">
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3"
                                  class="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:focus:ring-blue-400 dark:focus:border-blue-400 transition-colors duration-200"
                                  placeholder="{{ __('Optional description about this category') }}">{{ old('description') }}</textarea>
                    </div>
            ...
            <div class="mt-8 pt-6 border-t border-slate-200 dark:border-slate-800 flex justify-end space-x-3">
                <a href="{{ route('admin.media-categories.index') }}" class="px-4 py-2 text-slate-700 dark:text-slate-300 font-medium hover:text-slate-900 dark:hover:text-slate-100 transition-colors duration-200">
                    {{ __('Cancel') }}
                </a>
