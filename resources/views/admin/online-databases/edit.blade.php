...
<div class="max-w-4xl mx-auto space-y-4 px-4 md:px-6 py-4">
    ...
    <div class="flex items-center gap-3 bg-card border border-border p-3 rounded-md shadow-sm">
        ...
        <div>
            <h1 class="text-xl font-bold text-foreground">Cập nhật cơ sở dữ liệu trực tuyến</h1>
            <p class="text-xs text-muted-foreground mt-0.5">Chỉnh sửa thông tin cơ sở dữ liệu trực tuyến "{{ $database->title }}".</p>
        </div>
    </div>

    <!-- Form Section -->
    <div class="bg-card border border-border rounded-md shadow-sm p-4">
        <form action="{{ route('admin.online-databases.update', $database->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div class="space-y-1.5 md:col-span-2">
                    ...
                    <input type="text" name="title" id="title" required value="{{ old('title', $database->title) }}"
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="url" name="url" id="url" value="{{ old('url', $database->url) }}"
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="url" name="hd_url" id="hd_url" value="{{ old('hd_url', $database->hd_url) }}"
                ...
                <div class="space-y-1.5 md:col-span-2">
                    ...
                    @if($database->image_url)
                        <div class="mb-2 flex items-center gap-2">
                            <div class="w-16 h-8 bg-white border border-border rounded flex items-center justify-center p-0.5 overflow-hidden">
                                <img src="{{ $database->image_url }}" alt="Current Logo" class="max-w-full max-h-full object-contain">
                            </div>
                            <span class="text-xs text-muted-foreground">Logo hiện tại</span>
                        </div>
                    @endif
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        <div>
                            ...
                            <span class="text-[10px] text-muted-foreground">Tải ảnh mới từ thiết bị (jpeg, png, svg...)</span>
                        </div>
                        <div>
                            <input type="text" name="image_url" id="image_url" value="{{ old('image_url', $database->getRawOriginal('image_url')) }}"
                ...
                <div class="space-y-1.5 md:col-span-2">
                    ...
                    <textarea name="content" id="content" rows="4"
                              ...
                              placeholder="Nhập mô tả giới thiệu về cơ sở dữ liệu này...">{{ old('content', $database->content) }}</textarea>
                    @error('content')
                        <p class="text-xs text-destructive mt-1 font-semibold">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $database->sort_order) }}" min="0"
                           class="w-full px-3 py-1.5 bg-background border border-border text-foreground rounded-sm text-sm focus:ring-1 focus:ring-primary focus:border-primary">
                </div>

                <!-- Status Checkbox -->
                <div class="flex items-center space-x-2 mt-4 md:col-span-2">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $database->is_active) ? 'checked' : '' }}
