...
<div class="space-y-6">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-slate-100">{{ __('Edit Media Item') }}</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">{{ __('Update item in category: ') }} <span class="font-semibold text-blue-600 dark:text-blue-400">{{ $category->name }}</span></p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
        <form action="{{ route('admin.media-items.update', $mediaItem) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="link_url" 
                               class="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:focus:ring-blue-400 dark:focus:border-blue-400 transition-colors duration-200" 
                               value="{{ old('link_url', $mediaItem->link_url) }}">
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="link_target" 
                                    class="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:focus:ring-blue-400 dark:focus:border-blue-400 transition-colors duration-200">
                                <option value="_self" {{ old('link_target', $mediaItem->link_target) == '_self' ? 'selected' : '' }}>Tab hiện tại (_self)</option>
                                <option value="_blank" {{ old('link_target', $mediaItem->link_target) == '_blank' ? 'selected' : '' }}>Tab mới (_blank)</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <input type="number" name="sort_order" 
                                   class="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:focus:ring-blue-400 dark:focus:border-blue-400 transition-colors duration-200" 
                                   value="{{ old('sort_order', $mediaItem->sort_order) }}">
                        </div>
                    </div>
                    ...
                    <div class="flex items-center space-x-2 pt-2">
                        <input type="checkbox" name="is_active" id="is_active" 
                               ...
                               {{ old('is_active', $mediaItem->is_active) ? 'checked' : '' }}>
            ...
            <div class="mt-8 pt-6 border-t border-slate-200 dark:border-slate-800 flex justify-end space-x-3">
                <button type="submit" class="inline-flex items-center px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg shadow-lg shadow-blue-200 dark:shadow-none transition-all duration-200 hover:scale-[1.02]">
                    <i class="fas fa-save mr-2"></i>{{ __('Update Item') }}
                </button>
            </div>
        </form>
    </div>
