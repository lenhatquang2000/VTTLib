        ...
        <div x-show="open" x-collapse x-cloak class="border-t border-slate-200 dark:border-slate-700">
            <form action="{{ route('admin.site-nodes.layout-settings') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {{-- Book Intro Image (Section 4) --}}
                    <div class="space-y-2 md:col-span-2 pt-4 border-t border-slate-100 dark:border-slate-800">
                        <label class="text-[10px] text-indigo-500 dark:text-indigo-400 uppercase font-black tracking-widest">{{ __('Ảnh giới thiệu sách (Trang chủ)') }}</label>
                        @php $currentBookIntro = \App\Models\SystemSetting::get('book_intro_image'); @endphp
                        <div x-data="{ fileName: '', previewUrl: '{{ $currentBookIntro ? asset('storage/' . $currentBookIntro) : '' }}', removeImg: false }" class="space-y-3">
                            <div class="flex items-center gap-3">
                                ...
                                @if($currentBookIntro)
                                    <button type="button" @click="removeImg = !removeImg; previewUrl = removeImg ? '' : '{{ asset('storage/' . $currentBookIntro) }}'"
                                            class="px-3 py-3 rounded-xl text-sm transition-all"
