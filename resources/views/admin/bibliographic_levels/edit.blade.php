...
">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
            <h1 class="text-xl font-bold text-foreground tracking-tight">{{ __('Sửa kiểu biểu ghi') }}</h1>
            <p class="text-xs text-muted-foreground">{{ __('Cập nhật kiểu biểu ghi MARC') }}</p>
        </div>
    ...
    <div class="bg-card rounded-md shadow-sm border border-border p-4">
        <form method="POST" action="{{ route('admin.bibliographic-levels.update', $bibliographicLevel) }}" class="space-y-3">
            @csrf
            @method('PUT')
            ...
            <div class="flex items-center space-x-2 px-1 py-1">
                <label class="flex items-center space-x-2 cursor-pointer group">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $bibliographicLevel->is_active) ? 'checked' : '' }} 
