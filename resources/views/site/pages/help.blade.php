            ...
            <div class="flex flex-col md:flex-row justify-between items-end gap-8">
                ...
                <div class="hidden lg:block pb-2">
                    <nav class="flex items-center gap-4 text-slate-400 text-xs font-bold uppercase tracking-widest">
                        ...
                        <i class="fas fa-chevron-right text-[8px]"></i>
                        <span class="text-white">{{ $node->display_name }}</span>
                    </nav>
                </div>
            </div>
        </div>
        ...
        <div class="w-full px-4 md:px-12 lg:px-24">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
                            ...
                            <nav class="p-4">
                                @if($sidebarItems->count() > 0)
                                    @foreach($sidebarItems as $item)
                                        <a href="{{ $item->getUrl() }}" 
                                           class="group flex items-center justify-between px-6 py-4 rounded-2xl transition-all mb-1 {{ $node->id === $item->id ? 'bg-blue-600 text-white shadow-xl shadow-blue-600/20' : 'text-slate-600 hover:bg-slate-50 hover:text-blue-600' }}">
                                            <span class="font-bold text-sm">{{ $item->display_name }}</span>
                                            <i class="fas fa-chevron-right text-[10px] transition-transform group-hover:translate-x-1 {{ $node->id === $item->id ? 'text-white/50' : 'text-slate-300' }}"></i>
                                        </a>
                                    @endforeach
                                @else
                ...
                <main class="lg:col-span-9 space-y-12">
                    @if(!$node->parent_id)
                        {{-- 🌟 Hiển thị DANH MỤC CARD nếu là trang Hướng dẫn gốc --}}
                    ...
                    <div class="flex flex-col md:flex-row justify-between gap-6">
                        ...
                        <div class="w-full md:w-1/2">
                            @if($prev)
                                <a href="{{ $prev->getUrl() }}" class="flex items-center gap-6 p-6 bg-white rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all group">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-sm font-bold text-slate-900 group-hover:text-blue-600 transition-colors">{{ $prev->display_name }}</div>
                                    </div>
                                </a>
                            @endif
                        </div>
                        ...
                        <div class="w-full md:w-1/2">
                            @if($next)
                                <a href="{{ $next->getUrl() }}" class="flex items-center justify-end gap-6 p-6 bg-white rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all group text-right">
                                    <div>
                                        <div class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tiếp theo</div>
                                        <div class="text-sm font-bold text-slate-900 group-hover:text-blue-600 transition-colors">{{ $next->display_name }}</div>
                                    </div>
                                    <div class="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-400 group-hover:bg-blue-600 group-hover:text-white transition-all">
