...
@section('title', $node->meta_title ?: 'Trang Chủ - Thư viện số')
@section('meta-description', $node->meta_description ?: 'Hệ thống quản lý thư viện số hiện đại')
@section('meta-keywords', $node->meta_keywords ?: 'thư viện, tra cứu, sách, tài liệu')
            ...
            <div class="flex flex-col md:flex-row items-center justify-between gap-12">
                <div class="md:w-1/2 space-y-8 text-center md:text-left">
                    ...
                    <div class="flex flex-wrap justify-center md:justify-start gap-4 pt-4">
                        <a href="{{ route('site.page', 'tra-cuu-opac') }}" class="px-8 py-4 bg-white text-blue-900 font-bold rounded-full shadow-lg hover:bg-blue-50 transition-all transform hover:-translate-y-1">
                            Bắt đầu tra cứu
                        </a>
                        <a href="{{ route('news.index') }}" class="px-8 py-4 bg-transparent border-2 border-white/30 text-white font-bold rounded-full hover:bg-white/10 transition-all backdrop-blur-sm">
                            Xem tin tức
                        </a>
                    </div>
                </div>
        ...
        <div class="container mx-auto px-6 relative z-10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div class="group bg-white p-10 rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 hover:border-blue-100 relative overflow-hidden transform hover:-translate-y-2">
                    <div class="absolute -right-4 -top-4 w-24 h-24 bg-{{ $f['color'] }}-50 rounded-full group-hover:scale-150 transition-transform duration-500 opacity-50"></div>
                    <div class="w-16 h-16 bg-{{ $f['color'] }}-100 rounded-2xl flex items-center justify-center mb-8 relative z-10 group-hover:bg-{{ $f['color'] }}-600 group-hover:rotate-6 transition-all duration-300">
                        <i class="{{ $f['icon'] }} text-{{ $f['color'] }}-600 text-2xl group-hover:text-white transition-colors"></i>
                    </div>
                    <h4 class="text-2xl font-bold text-slate-900 mb-4 relative z-10">{{ $f['title'] }}</h4>
                    <p class="text-slate-600 leading-relaxed mb-6 relative z-10">{{ $f['desc'] }}</p>
                    <a href="{{ $f['route'] === 'home' ? '#' : (Route::has($f['route']) ? route($f['route'], $f['route'] === 'site.page' ? ['code' => 'dich-vu'] : []) : '#') }}" class="text-{{ $f['color'] }}-600 font-bold flex items-center gap-2 group-hover:gap-3 transition-all">
